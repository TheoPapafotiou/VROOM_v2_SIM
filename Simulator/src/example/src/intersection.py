...
import numpy as np
import time
from skimage import ransac, LineModelND
...
class Intersection:
    ...
    def mask(self,image,height,width,case):
        # for the straight 
        if case == 1:
            polygons=np.array([
        ...
        # for the small right turn 
            ...
            polygons=np.array([
                [(int(width/3), int(height*0.45)),(width,int(height*0.45)),(width,height*0.7),(int(width/3),height*0.7)] #(y,x)
                ])
    ...
    def small_right_turn(self,frame):
        ...
        maskedimg=self.mask(cannyimg,height,width,2)
        ...
        axes = (radius, radius)
        self.draw_ellipse(frame, center, axes, 0, start_angle, end_angle, 255)
        print("DURATION / FRAME", duration)

        return frame

    def straight(self,frame):
        counter = 0
        counter3 = 0
        self.img = frame 
        width = frame.shape[1]
        ...
        cannyimg = self.canny(frame)
        img = self.mask(cannyimg,height,width,1)
        ...
        if self.counter2 == 5:
            ...
            if self.y_points[counter] < height*0.6: 
                cv2.line(frame, (int(width*0.1), int(height)),(int(self.x_points[counter]), int(self.y_points[counter])), self.white, 9)
                cv2.line(frame, (int(width*0.9), int(height)),(int(width-self.x_points[counter]), int(self.y_points[counter])), self.white, 9)
                counter3 = 0
            else: 
                counter3 = counter3 + 1
                cv2.line(frame, (int(width*0.1), int(height)),(int(self.x_points[counter-counter3]), int(self.y_points[counter-counter3])), self.white, 9)
                cv2.line(frame, (int(width*0.9), int(height)),(int(width-self.x_points[counter-counter3]), int(self.y_points[counter-counter3])), self.white, 9)
